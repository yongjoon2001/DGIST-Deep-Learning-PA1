...
class ThreeLayerNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.layer3(x)  # Remove softmax - CrossEntropyLoss includes it
        return x
...
def plot_loss_graph(model, save_path):
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('Training and Test Loss (PyTorch)')
    plt.legend()
    plt.grid(True)
...
def plot_confusion_matrix(model, test_loader, save_path):
    ...
    plt.title('Confusion Matrix (PyTorch)', fontsize=16, weight='bold', pad=20)
...
def main():
    ...
    print("\nTraining 3-layer Neural Network (PyTorch)...")
    train_model(model, train_loader, test_loader, epochs=20, learning_rate=0.01)
    ...
    results_dir = "results/nn_framework"
    ...
    # Save model
    checkpoint_dir = "checkpoints/nn_framework"
    ...
    with open(metrics_path, 'w') as f:
        f.write("="*60 + "\n")
        f.write("3-Layer Neural Network (PyTorch) - Training Metrics\n")
        f.write("="*60 + "\n\n")
        ...
        f.write(f"  Output Layer: 10 neurons (CrossEntropyLoss)\n\n")
